...
public class Repository extends DBAdapter{
	...
	public boolean saveDiary(Diary diaryToDump) {
		...
		try {
			...
			mResult=false;
			Log.v(this.getClass().getCanonicalName(), "Errore nell' inserimento del Diary try to update");
			...
			mResult=false;
			Log.v(this.getClass().getCanonicalName(), "NullPointerException under dump Diary");
		} catch (PersistenceException e) {
			mResult=false;
			Log.v(this.getClass().getCanonicalName(), "PersistenceException under dump Diary");
		}finally{
			close();
		}
